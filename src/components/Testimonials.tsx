    ...
    <section className="p-6  w-full py-24 ">
      <div className="w-full h-full   max-w-[1440px] mx-auto relative">
        <Image
          ...
          width={100}
          height={100}
          className="object-cover absolute -top-24 lg:-top-36 left-0 w-24 h-24 invert"
        />
        <h1 className="text-2xl lg:text-6xl font-medium   lg:my-12">
          Inspirations
        </h1>
        <div className="flex flex-col md:flex-row items-start gap-8 py-6 w-full ">
          {testimonials.map(({ testimonial, image, name }, i) => (
            <div
              key={i}
            ...
            >
              ...
              <div
                className={`rounded-full border-t border-t[#1d31619a] group-hover:border-t-sky-500 w-16 h-16 object-cover absolute left-3 -top-8 z-20 bg-[#1d3161]`}
              >
                <Image
                  src={image}
                  alt={name}
