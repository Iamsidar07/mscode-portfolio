            ...
            <div className='w-full flex items-center justify-between relative max-w-[1440px] mx-auto '>
...

                {
                    ...
                    <div className='flex flex-col md:hidden  gap-4 absolute top-16 right-0 min-w-[200px] py-4  border border-gray-800 rounded-md z-50 shadow text-center bg-gray-900'>
                        {
                            navLinks.map((link, index) => (<Link onClick={() => setIsDropdownOpen(false)} className='pl-4 border-b border-gray-800  py-1' href={`/#${link.id}`} key={index}>
                                {link.title}
                            </Link>))
                        }
                        <Link onClick={() => setIsDropdownOpen(false)} target='_blank' className='pl-4 border-b border-gray-800 last:border-none' href={`https://personal-dev-blog-nu.vercel.app/`}>
                            Blog
                        </Link>
                    </div>
                }
